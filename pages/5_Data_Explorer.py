...
st.sidebar.markdown(
    """
    Explore transaction records, assess data quality,
    review metadata, and uncover revenue insights using
    interactive filters and detailed dataset exploration.
    """
)
# ---------------------------------------------------
# SIDEBAR FILTERS
...
if search_product:
    filtered = filtered[
        filtered["product_detail"].str.contains(search_product, case=False, na=False)
    ]
...
# 1. Custom High-Prominence Premium KPI Cards
...
with col1:
    st.markdown(
        ...
        <div style="
            ...
            text-align: left;
        ">
            <span style="color: #757575; font-size: 14px; font-weight: 700; text-transform: uppercase;"> Total Rows</span>
            <h2 style="color: {COLOR_BEAN}; margin: 8px 0 0 0; font-size: 28px; font-weight: 700;">{len(filtered):,}</h2>
        </div>
        """,
        unsafe_allow_html=True
    )
...
with col2:
    st.markdown(
        ...
        <div style="
            ...
            text-align: left;
        ">
            <span style="color: #757575; font-size: 14px; font-weight: 700; text-transform: uppercase;"> Total Revenue</span>
            <h2 style="color: {COLOR_BEAN}; margin: 8px 0 0 0; font-size: 28px; font-weight: 700;">${filtered['revenue'].sum():,.0f}</h2>
        </div>
        """,
        unsafe_allow_html=True
    )
...
with col3:
    st.markdown(
        ...
        <div style="
            ...
            text-align: left;
        ">
            <span style="color: #757575; font-size: 14px; font-weight: 700; text-transform: uppercase;">🛍 Unique Products</span>
            <h2 style="color: {COLOR_BEAN}; margin: 8px 0 0 0; font-size: 28px; font-weight: 700;">{filtered["product_id"].nunique()}</h2>
        </div>
        """,
        unsafe_allow_html=True
    )
...
# [The rest of your code - with st.expander("📋 Dataset Health Overview")... continues exactly the same here]
...
with st.expander("📋 Dataset Health Overview", expanded=False):
    ...
    st.markdown(
        """
        <div style="padding:12px 0px; color:#4E342E; font-size:15px; font-weight:500;">
            Quick assessment of dataset quality, completeness, structure, and overall data readiness for analysis.
        </div>
        """,
        unsafe_allow_html=True
    )
...
with tab1:
    ...
    with col2:
        selected_column = st.selectbox(
            "Column Summary Descriptive Focus",
            filtered.columns
        )
...
with tab2:
    ...
    st.caption("Columns with higher missing percentages may require cleaning or imputation before modeling.")
    ...
    st.caption("Schema validation ensures every field is stored in the correct format for analysis and reporting.")

    st.divider()
    st.subheader(" Dataset Quality Status")

    # Clean unhighlighted layout reporting blocks for Quality pass checking
    if missing_values == 0 and duplicate_rows == 0:
        ...
        status_text = f"<b>Dataset Warning:</b> {duplicate_rows:,} duplicate records detected. Deduplication recommended to preserve transaction accounting accuracy."
    else:
    ...
    st.markdown(
        ...
        ">
        <div style="font-size: 18px; color: #4E342E; line-height: 1.6; padding: 5px 0px;">
            ⚠️ {status_text}
        </div>
        """,
        unsafe_allow_html=True
...
with tab3:
    ...
    dictionary = pd.DataFrame({
        "Column": [
            "transaction_id", "transaction_date", "transaction_time", "store_location",
            "product_category", "product_type", "product_detail", "transaction_qty",
            "unit_price", "revenue", "hour", "day_name", "month_name"
        ],
        "Description": [
            "Unique transaction identifier", "Date of customer purchase", "Time of transaction",
            "Store branch location",
            "High-level product grouping", "Product sub-category", "Individual product SKU", "Units purchased",
            "Selling price per unit", "Total revenue generated", "Hour extracted from transaction time", "Day of week",
            "Month name"
        ],
        "Data Type": [
            "Integer", "Date", "Time", "Categorical", "Categorical", "Categorical", "Categorical", "Integer", "Float",
            "Float", "Integer", "Categorical", "Categorical"
        ],
        "Business Use": [
            "Transaction tracking", "Trend analysis", "Peak hour analysis", "Store performance analysis",
            "Category analysis", "Product mix analysis", "Pareto analysis", "Demand measurement",
            "Pricing analysis", "Revenue analysis", "Operational planning", "Weekly trend analysis",
    ...
    st.dataframe(dictionary, use_container_width=True, hide_index=True, height=500)
    st.markdown(
        f"<div style='font-size:14.5px; color:#757575; font-style:italic;'>📖 Metadata Catalog contains {len(dictionary)} documented fields used throughout the analytics dashboard.</div>",
        unsafe_allow_html=True)
...
with tab4:
    ...
    # Integrated with corporate Coffee Palette branding variables
    fig = px.histogram(
        filtered,
        x="revenue",
        nbins=30,
    ...
    fig.update_layout(
        ...
        xaxis_title="Revenue Check Volume ($)",
        yaxis_title="Transaction Frequency Count"
    )

    st.plotly_chart(fig, use_container_width=True, theme=None)

    # Non-highlighted inline description added below graph asset canvas
    st.markdown(
        """
        <div style="font-size: 16px; color: #4E342E; line-height: 1.6; margin-top: 5px; margin-bottom: 20px;">
             <b>Chart Analysis:</b> This distribution histogram details transaction check sizes. It identifies common baseline customer spending brackets and maps volume frequencies cleanly.
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    # ---------------------------------------------------
    # REVENUE DISTRIBUTION INSIGHTS (SIMPLE TEXT OVERHAUL)
    # ---------------------------------------------------
    st.markdown(
        f"""
        <div style="
            ...
            box-shadow: 0 4px 12px rgba(62, 39, 35, 0.04);
            ">
            <h3 style="margin: 0 0 14px 0; color: #3E2723; font-size: 1.50rem; font-weight: 700;">
                ⬩➤ Revenue Distribution Insights
            </h3>
            <div style="font-size: 18px; color: #4E342E; line-height: 1.8; display: flex; flex-direction: column; gap: 8px;">
                <div>
                     <b>Average Baseline:</b> The global average transaction value yields exactly <b>${filtered['revenue'].mean():.2f}</b> per customer visit.
                </div>
                <div style="border-top: 1px dashed #EFEBE9; padding-top: 8px;">
                     <b>Median Threshold:</b> 50% of entire filtered customer ticket bills generate less than <b>${q2:.2f}</b>.
                </div>
                <div style="border-top: 1px dashed #EFEBE9; padding-top: 8px;">
                     <b>Premium Segment:</b> High-ticket purchases surpassing the 75th percentile line of <b>${q3:.2f}</b> isolate your high-value target core segment.
                </div>
                <div style="border-top: 1px dashed #EFEBE9; padding-top: 8px;">
                     <b>Strategic Utility:</b> Mapping distribution variables empowers management to scale baseline menu prices, model promotion bundles, and design optimized retail assortments.
                </div>
            </div>
        </div>
        """,
